...
3. Get Credentials (Project Settings > API):
   ...
   - `SERVICE_ROLE_KEY`: Secret for server-side.
...
3. Install Docker Desktop; run it.
4. Initialize: `npx supabase init` (creates `supabase/` folder—commit to Git).
5. Start Local Stack: `npx supabase start` (runs Docker containers; outputs local creds like `http://localhost:54321` and keys).
...
- **Migrations Workflow**:
  1. Create: `npx supabase migration new <name>` (e.g., `create_products_table`).
  2. Generate SQL in Cursor AI: Prompt like "Create SQL for products table: id (BIGINT PK), name TEXT NOT NULL, price NUMERIC DEFAULT 0, created_at TIMESTAMPTZ DEFAULT NOW()".
...
- Best Practices: Never expose service_role key; use AI to learn/accelerate; practice on small projects.
...
1. **Create a .cursorrules.md File**:
   ...
   - Example content (copy-paste and customize):
     ...
     - Integrations: Use `@supabase/mcp-server-supabase` if installed for direct DB queries/management.
     ```
...
3. **Install and Integrate `@supabase/mcp-server-supabase`**:
   - MCPs (e.g., from GitHub supabase-community/supabase-mcp) are 2025 tools that connect AI assistants like Cursor to your Supabase project.
   - Install: In terminal, `npm install @supabase/mcp-server-supabase --save-dev`.
...
6. **Terminal Aliases for CLI**:
   - In your shell (.zshrc or .bash_profile): Add aliases like `alias sb-start='npx supabase start'` , `alias sb-reset='npx supabase db reset'`.
...
3. **Integrate `@supabase/mcp-server-supabase` with Claude**:
...
- **Resources**: Watch "Supabase + Cursor: 10X Your Database Workflow" on YouTube for visuals. Check the official `@supabase/mcp-server-supabase` GitHub for updates.
- **Safety**: Configure AI rules to always confirm prod changes (e.g., "Ask user before db push").

These setups make your AI tools more self-sufficient, handling 95%+ of DB work with minimal input. Start with .cursorrules and `@supabase/mcp-server-supabase` for immediate impact.
